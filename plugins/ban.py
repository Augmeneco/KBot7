    ...
    def execute(self,msg): 
        ...
        if msg['text_split'][1] == 'бан':
            ...
            if len(msg['text_split']) >= 3:
                db_cur.execute('UPDATE users SET perm=0 WHERE id='+str(userid))
                ...
                db_cur.execute('UPDATE users SET data=\'{0}\' WHERE id={1}'.format(json.dumps(msg['userdata']),userid))
                db.commit()
        ...
        if msg['text_split'][1] == 'разбан':
            userid = message['from_id']
            db_cur.execute('UPDATE users SET perm=1 WHERE id='+str(userid))
            db.commit()
